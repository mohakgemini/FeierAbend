...
export function useYouTubePlayer(onEnded: () => void) {
  const player = useRef<any>(null); const endedRef = useRef(onEnded); const playlistMode = useRef(false); const [ready, setReady] = useState(false); const [playing, setPlaying] = useState(false); const [currentTime, setCurrentTime] = useState(0); const [duration, setDuration] = useState(0); const [videoData, setVideoData] = useState<{title?:string;author?:string;video_id?:string}>({});
  ...
  const initPlaylist = useCallback((playlistId:string) => {
    ...
    const cueRandom = (target:any) => {
      ...
      const choose = setInterval(() => {
        ...
        } else if (++attempts > 20) clearInterval(choose);
      }, 150);
    };
  ...
  useEffect(() => { const timer = setInterval(() => { if (player.current?.getCurrentTime) { setCurrentTime(player.current.getCurrentTime() || 0); setDuration(player.current.getDuration() || 0); setVideoData(player.current.getVideoData?.() || {}) } }, 500); return () => clearInterval(timer) }, []);
  return { init, initPlaylist, ready, playing, currentTime, duration, videoData, play: () => player.current?.playVideo(), pause: () => player.current?.pauseVideo(), seek: (seconds:number) => player.current?.seekTo(seconds, true), nextVideo:()=>player.current?.nextVideo(), previousVideo:()=>player.current?.previousVideo(), shufflePlaylist:()=>{player.current?.setShuffle(true);player.current?.nextVideo()} };
}
